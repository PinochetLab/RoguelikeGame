...
internal static class CellHelpers
{
    ...
    {
        var template = cell.Type switch
        {
            CellType.Room => FillRoom(cell.Openings, parameters),
            CellType.Corridor => FillCorridor(cell.Openings, parameters),
            _ => null
        };
    ...
    private static void CarveCorridors(Direction direction, Tile[,] template, GeneratorParams parameters,
        bool fillWalls = false)
    {
    ...
    private static Tile[,] MakeTemplate(int size, Tile tileType = default)
    {
